...
if ! is_license_valid; then
    echo "Lisensi tidak valid atau belum dimasukkan!"
    if [[ ! -f "$LICENSE_FILE" ]]; then
...
echo -e "\033[31m"
echo " ╾━╤デ╦︻"
echo " IM LEXCZ FROM INDONESIA HACKER"
echo " ╾━╤デ╦︻"
echo " IM LEXCZ FROM INDONESIA HACKER"
echo -e "\033[0m"
...
echo "7. HAPUS BACKROUND PTERODACTYL (RESET BACKROUND JADI YANG AWAL)"
echo "8. HAPUS THEME/ADDON"
echo "9. MATIKAN SEMUA ANIMASI INSTALLER (TIDAK BERLAKU DI WEB ANDA HANYA MEMATIKAN TEXT ANIMATION INSTALLER)"
echo "10. KELUAR DARI INSTALLER"
read -p "PILIH OPSI (1-10): " OPTION
case "$OPTION" in
    1)
...
if [ $? -eq 0 ]; then
    ...
    echo "TERJADI KESALAHAN SAAT MEMPERBARUI FILE!! SILAHKAN HUBUNGI 087743212449 UNTUK MEMINTA BANTUAN"
fi
 ;;
    5)
     # Masukkan token GitHub langsung di sini
